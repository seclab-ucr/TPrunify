...
public:
  ...
  Value *VisitCallExpr(const CallExpr *E) {
    ...
    static PrintingPolicy print_policy(CGF.getContext().getLangOpts());
    ...
    E->printPretty(stream, nullptr, print_policy);
    stream.flush();

    //llvm::errs()<<"VisitCallExpr: "<<expr_string<<"\n";
...
Value *ScalarExprEmitter::VisitCastExpr(CastExpr *CE) {
  ...
  case CK_BaseToDerived: {
	...
	Derived.getPointer()->print(rso);
      llvm::errs()<<"CastExpr For " << out_str << "\n";
      llvm::errs()<<"1Casting Inst : "<<insStr<<"\n";
      if (insStr.find("trackedtype.std::") == std::string::npos) {
	    if (!NonVirtualOffset)
    ...
    ////llvm::errs()<<"1Casting Inst : "<<*Derived.getPointer()<<"\n";
