  ...
  case CK_BaseToDerived: {
    ...
    if (SanOpts.has(SanitizerKind::HexType)) {
      ...
      E->printPretty(outstream, NULL, PrintingPolicy(lo));
			outstream.flush();
      ////llvm::errs()<<"0CastExpr For " << out_str << "\n";
      if (out_str.find("static_cast<") != std::string::npos && out_str.find("std::") != std::string::npos) {
      }
      else {
	      if (!NonVirtualOffset)
    ...
    if (SanOpts.has(SanitizerKind::CFIDerivedCast))
      EmitVTablePtrCheckForCast(E->getType(), Derived.getPointer(),
                                ...
                                E->getBeginLoc());
    LValue lr = MakeAddrLValue(Derived, E->getType(), LV.getBaseInfo(),
                          CGM.getTBAAInfoForSubobject(LV, E->getType()));
    ////llvm::errs()<<"0Casting Inst : "<<*lr.getAddress(*this).getPointer()<<"\n";
    return lr;//MakeAddrLValue(Derived, E->getType(), LV.getBaseInfo(),
                //          CGM.getTBAAInfoForSubobject(LV, E->getType()));
  }
...
RValue CodeGenFunction::EmitCall(QualType CalleeType, const CGCallee &OrigCallee,
  ...
  /*llvm::errs()<<"RValue :\n";
  if (Call.isScalar()) {
    llvm::errs()<<*Call.getScalarVal()<<"\n";
    /home/yzhai003/HexType-14.0.5/llvm-14.0.5.src/tools/clang/lib/AST/ExprClassification.cpp
  }*/
