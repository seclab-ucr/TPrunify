...
std::unordered_set<std::string> llvm_safe_casts {
...
void CodeGenFunction::EmitHexTypeCheckForchangingCast(QualType T,
		...
		SourceLocation Loc) {
    std::string locInfo = Loc.printToString(getContext().getSourceManager());
    llvm::errs()<<"0LocStr : "<<locInfo<<"\n";

    size_t pos = locInfo.find(":");
    if (pos != std::string::npos) {
      std::string filename = locInfo.substr(0, pos);
      std::string rest=locInfo.substr(pos+1);
      pos = rest.find(":");
      if (pos != std::string::npos) {
        std::string lineNo = rest.substr(0, pos);
        std::string key_str = filename+"##"+lineNo;
	//llvm::errs()<<"key_str = "<<key_str<<"\n";
        if (llvm_safe_casts.find(key_str) != llvm_safe_casts.end()) {
          llvm::errs()<<"safe cast : file: "<<filename<<", linNo : "<<lineNo<<"\n";
          return;
        }
     }
    }

    ////llvm::errs()<<"0Inside EmitHexTypeCheckForchangingCast: \n";
	if (llvm::Value *DstTyHashValue = getHashValueFromQualType(T)) {
    ////llvm::errs()<<"0DstTyHashValue = getHashValueFromQualType.\n";
		llvm::Value *DynamicArgs[] = { Base, Derived };
    llvm::errs()<<"Insert __type_casting_verification_changing :  "<<DstTyHashValue<<"\n";
...
void CodeGenFunction::EmitHexTypeCheckForCast(QualType T,
		...
		SourceLocation Loc) {
  std::string locInfo = Loc.printToString(getContext().getSourceManager());
  llvm::errs()<<"LocStr : "<<locInfo<<"\n";
  ////llvm::errs()<<"Inside EmitHExTypeCheckForCast :\n";
  size_t pos = locInfo.find(":");
  if (pos != std::string::npos) {
    std::string filename = locInfo.substr(0, pos);
    std::string rest=locInfo.substr(pos+1);
	...
	if (llvm::Value *DstTyHashValue = getHashValueFromQualType(T)) {
    ///llvm::errs()<<"getHashValueFromQualType :\n";
		llvm::Value *DynamicArgs[] = { Derived };
    llvm::errs()<<"Insert __type_casting_verification. : "<<DstTyHashValue<<"\n";
...
void CodeGenFunction::EmitHexTypeReinterpretCast(QualType T,
	...
	if (auto *ClassTy = T->getAs<RecordType>()) {
		...
		llvm::errs()<<"insert reinterpretcast\n";
