...
CodeGenFunction::~CodeGenFunction() {
  ...
  if (ClEmitClangTypeInfo) {
    ...
    strcpy(fileName, "/name_to_hash.txt");
    for (auto item : HashToType) {
      sprintf(tmp, "Name : %lu, Hash : %s", item.first, item.second.c_str());
      HexTypeUtil.writeInfoToFile(tmp, fileName);
    }
  }
  else {
    //llvm::errs()<<"No ClEmitClangTypeInfo passed in. \n";
  }
}
